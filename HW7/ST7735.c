...
void LCD_drawChar(unsigned short x2, unsigned short y2, char mess, unsigned short colorOn, unsigned short colorOff){
    int col = 0;
    int j = 0;
    for(col=0; col<5; col++) {
        unsigned short pixels = ASCII[mess - 0x20][col];
        for(j=0; j<8; j++){
            if ((pixels >>j) & 1) {
                LCD_drawPixel(x2+col,y2+j,colorOn);
            } 
            else {
                LCD_drawPixel(x2+col,y2+j,colorOff);
            }
        }   
    }
}
...
void LCD_drawString(unsigned short x3, unsigned short y3, char message[10], unsigned short color1, unsigned short color2){
    int i=0;
    while(message[i]){
        LCD_drawChar((x3 + 5*i), y3, message[i], color1,color2);
        i++;
    }
...
void LCD_drawProgressBox(unsigned short x4, unsigned short y4, unsigned short h, unsigned short length1, unsigned short length2, unsigned short color3, unsigned short color4){
    int k = 0;
    int l = 0;
    for(k=0;k<length1;k++){
        for(l=0;l<h;l++){
            LCD_drawPixel(x4+k,y4+l,color3);
        }
    }
    
    for(k=0;k<length2;k++){
        for(l=0;l<h;l++){
            LCD_drawPixel(x4+length1+k,y4+l,color4);
        }
    }
...
void LCD_drawProgressBoxVertical(unsigned short x4, unsigned short y4, unsigned short w, unsigned short height1, unsigned short height2, unsigned short color3, unsigned short color4){
    int k = 0;
    int l = 0;
    for(k=0;k<height1;k++){
        for(l=0;l<w;l++){
            LCD_drawPixel(x4+l,y4+k,color3);
        }
    }
    
    for(k=0;k<height2;k++){
        for(l=0;l<w;l++){
            LCD_drawPixel(x4+l,y4+height1+k,color4);
        }
    }
